	...
	class HarmonyPatch_Skills_SetupProfession {
		...
		private static void Postfix(Skills __instance, ProfessionType profession, SkillTree panel) {
			try {
				...
				reset_button.transform.position = _skillPointsTMP.transform.position + Vector3.down * _skillPointsTMP.GetComponent<RectTransform>().rect.height * 2;
				label.fontSize = 12;
				label.text = "[Unlimited Skill Points]";
